...
wget -O /dev/stdout -a /dev/null \
	"http://support.euro.dell.com/support/topics/topic.aspx/emea/shared/support/my_systems_info/en/details?servicetag=$SVTAG" \
	| awk 'BEGIN {RS="</tr>"; FS="</td>"} /Next Business Day|Mission Critical/ {print $4}' \
	| sed -e "s#.*\([0-9]\{2\}/[0-9]\{2\}/20[0-9]\{2\}\).*#\1#" | head -n 1 #tr '\n' ';'
